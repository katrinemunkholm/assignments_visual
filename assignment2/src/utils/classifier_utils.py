...
def plot_sample(X, y, classes, samples_per_class):
    ...
    for idx_cls, cls in enumerate(classes):
        ...
        for i, idx in enumerate(idxs):
            ...
            p = sns.heatmap(np.reshape(X[idx], (32,32)), cmap=plt.cm.gray, 
...
def plot_coefs(coefficients, nclasses):
    ...
    for i in range(nclasses):
        ...
        p = plt.imshow(coefficients[i].reshape(32, 32),
...
def plot_individual(X, y, sample_idx):
    ...
    """
    #plotting image
    plt.imshow(X[sample_idx].reshape(32,32), cmap='gray')
...
def prediction_coefficients(image, model, classes):
    ...
    for i in range(nclasses):
        ...
        p = plt.imshow(model.coef_[i].reshape(32, 32),
                      cmap=plt.cm.RdBu, vmin=-scale, vmax=scale);
        p = plt.title('Class %i' % i);
        p = plt.axis('off')
    ...
    for i in range(nclasses):
        ...
        p = plt.imshow(image*model.coef_[i].reshape(32, 32),
